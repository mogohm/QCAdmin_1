...
function reasonFrom(scoreResult, sop, adminText) {
  ...
  if (scoreResult.slaException) r.push("ℹ️ อยู่ในช่วง System Event — Response time ไม่ถูกหักเต็ม");
  if (scoreResult.isFatal)
    r.push("พบข้อผิดพลาดร้ายแรง (Fatal): " + (scoreResult.fatalReasons || []).map((x) => x.name).join(", "));
  if (d.responseTime != null && d.responseTime < 80 && !scoreResult.slaException)
  ...
  if (d.communicationTone != null && d.communicationTone < 70)
    r.push("น้ำเสียงควรสุภาพและแสดงความเข้าใจลูกค้ามากขึ้น เติมคำว่า ค่ะ/ขออภัย/เข้าใจ (Communication & Tone)");
  if (d.greetingClosing != null && d.greetingClosing < 65)
    r.push("ควรทักทาย/ปิดการสนทนาให้เรียบร้อย เช่น ยืนยันว่าดำเนินการแล้ว/สอบถามเพิ่มเติม (Greeting & Closing)");
  if (d.creditDepositWithdraw != null && d.creditDepositWithdraw < 70)
    r.push("ขั้นตอนฝาก/ถอนยังไม่ครบ ควรแจ้งลิงก์/ยอด/ขอสลิป ตาม SOP (Credit deposit/withdraw)");
