...
function dimGreetingClosing(a) {
  const g = hitWords(a, GREETING),
    ...
    p = hitWords(a, POLITE);
  const raw = clamp(60 + c.length * 15 + g.length * 10 + Math.min(p.length, 2) * 8);
  return {
    raw,
...
function dimResponseTime(sec, limitMin, slaException) {
  ...
  return {
    raw,
    evidence: { response_seconds: sec, limit_seconds: limit, sla_exception: !!slaException, detail: note },
...
function dimSignal(adminText, sop, signals, label) {
  ...
  return {
    raw,
    evidence: { matched_signals: hits, sop: sop ? { id: sop.id, topic: sop.topic } : null },
...
function scoreReply(input) {
  ...
  const compute = {
    ...
    upsellPromotion: () =>
      dimSignal(
        adminText,
        sop,
        ["โปร", "โบนัส", "เงื่อนไข", "ยอด", "เครดิต", "รับเพิ่ม", "สิทธิ", "คุ้ม", "แนะนำ"],
        "Upsell",
      ),
    creditDepositWithdraw: () =>
      dimSignal(
        adminText,
        sop,
        ["ลิงก์", "ลิ้ง", "bit.ly", "ยอด", "สลิป", "บัญชี", "โอน", "auto", "รอสักครู่", "ตรวจสอบ", "ธนาคาร"],
        "Deposit/Withdraw",
      ),
    kycProcess: () =>
      dimSignal(
        adminText,
        sop,
        ["ยืนยัน", "บัตร", "อีเมล", "email", "รหัส", "ปลดล็อค", "ตรวจสอบ", "เอกสาร", "kyc"],
        "KYC",
      ),
  };
  ...
  for (const d of details)
    if (d.applicable && d.pass === false && d.fail_reason && !["minorError"].includes(d.category_code)) {
