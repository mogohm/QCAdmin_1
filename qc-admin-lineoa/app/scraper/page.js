...
export default function ScraperPage() {
  ...
  const loadJobs = async () => {
    try {
      ...
      const justDone = newJobs.find(j =>
        j.status === 'done' && prev.find(p => p.id === j.id && p.status !== 'done')
      );
  ...
  function enableSchedule() {
    ...
    setCfg(newCfg); setCountdown(intervalMs); setMsg('');
  ...
  function disableSchedule() {
    ...
    localStorage.setItem('qc_schedule', JSON.stringify(newCfg));
    setCfg(newCfg); setMsg('⏹️ ปิด Auto-Schedule แล้ว');
  }
    ...
    <div className="shell">
      ...
      <main className="main">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 20 }}>
          ...
          <div className="card" style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
              ...
              {scheduleOn && (
                <span style={{ fontSize: 12, background: '#dcfce7', color: '#16a34a', padding: '2px 8px', borderRadius: 12, fontWeight: 600 }}>ACTIVE</span>
              )}
            </div>
            {scheduleOn ? (
              <div>
          ...
          {jobs.length === 0
            ...
            : (
              <table className="table">
                <thead>
                  <tr><th>เวลา</th><th>ช่วงวันที่</th><th>สถานะ</th><th>ห้อง chat</th><th>ข้อความ</th><th>ใช้เวลา</th><th>หมายเหตุ</th></tr>
                </thead>
                <tbody>
                  {jobs.map(j => {
                    ...
                    return (
                      <tr key={j.id}>
                        <td style={{ fontSize: 11, color: '#888', whiteSpace: 'nowrap' }}>{new Date(j.created_at).toLocaleString('th-TH')}</td>
                        <td style={{ fontSize: 12 }}>{j.date_from} — {j.date_to}</td>
                        <td><span style={{ color: statusColor(j.status), fontWeight: 600, fontSize: 12 }}>{statusLabel(j.status)}</span></td>
                        <td>{j.total_chats || '—'}</td>
                        <td style={{ fontWeight: j.logged_count > 0 ? 600 : 400, color: j.logged_count > 0 ? '#16a34a' : '#999' }}>{j.logged_count || '—'}</td>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
          ...
          <span style={{ fontSize: 16, fontWeight: 700, color: '#374151', whiteSpace: 'nowrap' }}>📊 Scrape Report</span>
          <div style={{ flex: 1, height: 1, background: '#e5e7eb' }} />
        </div>
        ...
        <div className="card" style={{ marginBottom: 20 }}>
          <div style={{ display: 'flex', gap: 12, alignItems: 'flex-end', flexWrap: 'wrap' }}>
            ...
            <button onClick={() => loadReport(dateFrom, dateTo)} disabled={reportLoading}
              style={{ padding: '8px 20px', fontWeight: 700, opacity: reportLoading ? 0.5 : 1, cursor: reportLoading ? 'not-allowed' : 'pointer' }}>
              {reportLoading ? '⏳ โหลด...' : '🔍 ดูรายงาน'}
            </button>
            <button onClick={() => setShowDiagram(v => !v)}
              style={{ padding: '8px 14px', background: '#f1f5f9', border: '1px solid #d1d5db', borderRadius: 6, cursor: 'pointer', fontSize: 13, color: '#374151' }}>
          ...
          <>
            ...
            {reportData.jobs?.length > 0 && (
              <div className="card" style={{ marginBottom: 20 }}>
                ...
                <table className="table">
                  <thead>
                    <tr><th>วันที่สร้าง</th><th>ช่วง</th><th>สถานะ</th><th>Chat</th><th>Messages</th><th>ใช้เวลา</th></tr>
                  </thead>
                  <tbody>
                    {reportData.jobs.map(j => {
                      const sec = j.started_at && j.finished_at
                        ? Math.round((new Date(j.finished_at) - new Date(j.started_at)) / 1000) : null;
                      const clr = { done: '#22c55e', running: '#3b82f6', error: '#ef4444', cancelled: '#9ca3af', pending: '#f59e0b' }[j.status] || '#888';
                      return (
                        <tr key={j.id}>
                          <td style={{ fontSize: 11, color: '#888' }}>{fmtTs(j.started_at || j.date_from)}</td>
                          <td style={{ fontSize: 12 }}>{j.date_from} — {j.date_to}</td>
                          <td><span style={{ color: clr, fontWeight: 700, fontSize: 12 }}>{j.status}</span></td>
                          <td>{j.total_chats || '—'}</td>
              ...
              {!reportData.customers?.length
                ...
                : reportData.customers.map(c => {
                    ...
                    const avgScr = c.messages.length
                      ...
                      : null;
                    return (
                      <div key={c.line_user_id} style={{ border: '1px solid #e5e7eb', borderRadius: 10, marginBottom: 12, overflow: 'hidden' }}>
                        <div onClick={() => toggle(c.line_user_id)} style={{
                          ...
                          <div style={{ display: 'flex', gap: 16, alignItems: 'center', fontSize: 12 }}>
                            ...
                            {avgScr !== null && (
                              <span style={{ background: scoreColor(avgScr), color: '#fff', padding: '2px 8px', borderRadius: 8, fontWeight: 700, fontSize: 12 }}>
                                Score {avgScr}
                              </span>
                            )}
                        ...
                        {open && (
                          <div style={{ padding: '16px', background: '#fff' }}>
                            {c.messages.length > 0 && (
                              <>
                                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10, borderLeft: '3px solid #f59e0b', paddingLeft: 10 }}>
                                    ...
                                    <div key={mi} style={{ background: '#f8fafc', borderRadius: 8, padding: 12 }}>
                                      {msg.customer_text && (
                                        <div style={{ display: 'flex', gap: 10, marginBottom: 8 }}>
                                          <div style={{ background: '#e5e7eb', borderRadius: 10, padding: '6px 12px', maxWidth: '70%', fontSize: 13 }}>
                                      ...
                                      <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10 }}>
                                        <div style={{ background: '#1d4ed8', color: '#fff', borderRadius: 10, padding: '6px 12px', maxWidth: '70%', fontSize: 13 }}>
                            ...
                            {c.notes.length > 0 && (
                              <>
                                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10, borderLeft: '3px solid #a855f7', paddingLeft: 10 }}>
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                                  {c.notes.map((n, ni) => (
                                    <div key={ni} style={{ background: '#faf5ff', border: '1px solid #e9d5ff', borderRadius: 8, padding: 12 }}>
                                      <pre style={{ margin: 0, fontSize: 12, whiteSpace: 'pre-wrap', color: '#1e293b', fontFamily: 'inherit' }}>{n.note_text}</pre>
