...
export default function RulesPage() {
  ...
  useEffect(() => {
    if (apiKey) localStorage.setItem("qc_api_key", apiKey);
  }, [apiKey]);
  ...
  const save = async () => {
    ...
    if (!body.rule_code || !body.rule_name) {
      setMsg({ ok: false, text: "กรุณากรอก Rule Code และ Rule Name" });
      return;
    }
                ...
                {rules.map((rule, i) => (
                  ...
                  >
                    ...
                    <td style={{ padding: "12px 16px" }}>
                      <span
                        style={{
                          background: catColor[rule.category] || "#6b7280",
                          color: "#fff",
                          borderRadius: 6,
                          padding: "2px 10px",
                          fontSize: 12,
