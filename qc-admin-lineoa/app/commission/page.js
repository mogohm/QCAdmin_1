...
const TIERS = [
  ["Excellent", "90-100", 1.2, "#16a34a"],
  ["Standard", "80-89", 1.0, "#0b5cab"],
...
export default function Commission() {
  ...
  const setOv = (id, v) => {
    ...
    if (v === "") delete n[id];
    setOverride(n);
    localStorage.setItem("commission_override", JSON.stringify(n));
  };
  ...
  const actions = (
    <>
      ...
      <button onClick={saveDB} style={{ background: "#0b5cab" }}>
        💾 บันทึกลง DB
      </button>
    </>
  );
                  ...
                  <tr key={a.admin_id}>
                    ...
                    <td>
                      <input
                        type="number"
                        placeholder="—"
                        value={override[a.admin_id] ?? ""}
                        onChange={(e) => setOv(a.admin_id, e.target.value)}
