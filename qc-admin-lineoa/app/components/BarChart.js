...
export default function BarChart({ rows = [], horizontal = true, unit = "", height = 18 }) {
      ...
      <div>
        {rows.map((r, i) => (
          <div key={i} style={{ margin: "8px 0" }}>
            ...
            <div style={{ background: "rgba(125,211,252,0.1)", borderRadius: 6, height }}>
              <div
                style={{
                  width: ((Number(r.value) || 0) / mx) * 100 + "%",
                  height,
                  borderRadius: 6,
                  background: r.color || "linear-gradient(90deg,#1f6feb,#38bdf8)",
    ...
    <div style={{ display: "flex", alignItems: "flex-end", gap: 10, height: 140, paddingTop: 10 }}>
      {rows.map((r, i) => (
        <div key={i} style={{ flex: 1, textAlign: "center" }}>
          ...
          />
          <div style={{ fontSize: 11, marginTop: 4, color: "#dbe7ff", fontWeight: 700 }}>{r.value}</div>
