...
function tryParseObj(v) {
  try {
    return typeof v === "object" ? v : JSON.parse(v) || null;
  } catch {
    return null;
  }
}
...
export default function ChatModal({ user, onClose }) {
  ...
  const submitDispute = async (qcScoreId) => {
    if (!disputeReason.trim()) {
      setDisputeMsg("กรอกเหตุผล");
      return;
    }
    const r = await fetch("/api/qc-disputes", {
      method: "POST",
            ...
            {avgScore != null && (
              <div
                style={{
                  fontWeight: 700,
                  color:
                    scoreColor(avgScore) === "#22c55e"
                      ? "#c8ffb0"
                      : scoreColor(avgScore) === "#f59e0b"
                        ? "#ffe4a0"
                        : "#ffcdd2",
                }}
              >
            ...
            return (
              <div key={msg.id}>
                ...
                {isAdmin &&
                  ...
                  (() => {
                    const dims =
                      (typeof msg.dimension_scores === "object"
                          ...
                          <div style={{ marginTop: 8, fontSize: 11 }}>
                            ...
                            <span
                              style={{
                                fontWeight: 700,
                                color:
                                  msg.dispute_status === "approved"
                                    ? "#16a34a"
                                    : msg.dispute_status === "rejected"
                                      ? "#dc2626"
                                      : "#f59e0b",
                              }}
                            >
