...
export async function POST(req) {
  ...
  const rows = await query`SELECT id, username, password_hash, role, display_name, qc_admin_id, status, is_active
  ...
  if (u.is_active === false || u.status === "disabled")
    return Response.json({ error: "บัญชีถูกปิดใช้งาน — ติดต่อผู้ดูแลระบบ" }, { status: 403 });
  if (u.status === "pending")
    return Response.json({ error: "บัญชีรออนุมัติ — รอผู้ดูแลระบบอนุมัติก่อนเข้าใช้งาน" }, { status: 403 });
