...
export async function POST(req) {
  ...
  if (String(b.password).length < 6)
    return Response.json({ error: "รหัสผ่านอย่างน้อย 6 ตัว" }, { status: 400 });
  const role = ALLOWED_ROLES.includes(b.requested_role)
    ? b.requested_role
    : "admin";
  ...
  try {
    ...
    const exists =
      await query`SELECT 1 FROM app_users WHERE username = ${username}`.catch(
        () => [],
      );
    if (exists[0])
    ...
    if (dupReq[0])
      return Response.json(
        { error: "มีคำขอ pending ของ username นี้อยู่แล้ว" },
        { status: 409 },
      );
    ...
    return Response.json({
      ok: true,
      request: rows[0],
      message: "ส่งคำขอแล้ว รอผู้ดูแลระบบอนุมัติ",
    });
  } catch (e) {
