...
export async function POST(req) {
  ...
  if (!conversation_id || !admin_id || !text)
    return Response.json(
      { error: "conversation_id, admin_id, text required" },
      { status: 400 },
    );
  ...
  const adminMsg =
    await query`INSERT INTO messages(conversation_id,line_user_id,admin_id,direction,message_text)
  ...
  if (lastCustomer[0]) {
    ...
    const an =
      await query`SELECT member_name FROM qc_admins WHERE id=${admin_id}`;
