...
export async function POST(req) {
  ...
  const conv = await query`SELECT c.*, lc.line_user_id, lc.display_name FROM conversations c JOIN line_customers lc ON lc.line_user_id=c.line_user_id WHERE c.id=${conversation_id}`;
  ...
  if (lastCustomer[0]) {
    ...
    qc = await runQc({
      conversationId: conversation_id, customerMessageId: lastCustomer[0].id, adminMessageId: adminMsg[0].id,
      adminId: admin_id, lineUserId: conv[0].line_user_id,
      customerText: lastCustomer[0].message_text, adminText: text, responseSeconds: diff[0].sec,
      createdAt: adminMsg[0].created_at, adminName: an[0]?.member_name, customerName: conv[0].display_name,
