...
export async function POST(req) {
  ...
  const msgs =
    await query`SELECT id FROM messages WHERE created_at = ${noonUtc}::timestamptz`;
  ...
  if (ids.length) {
    ...
    const m =
      await query`DELETE FROM messages WHERE id = ANY(${ids}) RETURNING id`;
  ...
  return Response.json({
    ...
    date,
    noonUtc,
    deleted: { qc_scores: deletedScores, messages: deletedMsgs },
  });
}
