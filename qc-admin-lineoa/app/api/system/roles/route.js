...
export async function GET(req) {
  ...
  if (g) return g;
  const roles =
    await query`SELECT role_key, role_name, is_system FROM roles ORDER BY role_key`.catch(
      () => [],
    );
  const rp =
    await query`SELECT role_key, permission_key FROM role_permissions`.catch(
      () => [],
    );
  const byRole = {};
  ...
  const list = (
    ...
    ...r,
    permissions: byRole[r.role_key] || ROLE_PERMS[r.role_key] || [],
  }));
