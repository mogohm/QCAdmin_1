...
export async function PATCH(req, { params }) {
  ...
  try {
    const r = (
      await query`SELECT * FROM user_registration_requests WHERE id = ${id}`
    )[0];
    ...
    if (b.action === "approve") {
      ...
      const exists =
        await query`SELECT 1 FROM app_users WHERE username = ${r.username}`;
      if (exists[0])
        return Response.json(
      ...
      await audit(me?.uid, null, "registration.approve", {
        username: r.username,
        role,
      });
    ...
    if (b.action === "reject") {
      ...
      await audit(me?.uid, null, "registration.reject", {
        username: r.username,
      });
