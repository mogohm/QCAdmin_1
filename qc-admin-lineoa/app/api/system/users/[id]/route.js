...
export async function PATCH(req, { params }) {
  ...
  try {
    const cur =
      await query`SELECT id, username, role, status FROM app_users WHERE id = ${id}`;
    if (!cur[0]) return Response.json({ error: "not found" }, { status: 404 });

    // กันปิด system_admin คนสุดท้าย / ปิดบัญชีตัวเอง
    if (
      (b.action === "disable" || b.status === "disabled") &&
      cur[0].role === "system_admin"
    ) {
      if (me?.uid === cur[0].id)
        return Response.json(
          { error: "ปิดบัญชีตัวเองไม่ได้" },
          { status: 400 },
        );
      const actives =
      ...
      if ((actives[0]?.n || 0) === 0)
        return Response.json(
          { error: "ปิด system_admin คนสุดท้ายไม่ได้" },
          { status: 400 },
        );
    }

    // เปลี่ยนรหัสผ่าน: new_password = ตั้งเอง, action 'reset' = สุ่มรหัสชั่วคราว
    if (b.action === "reset" || b.new_password) {
      ...
      await audit(
        me?.uid,
        id,
        b.new_password ? "user.set_password" : "user.reset_password",
        {},
      );
      return Response.json({
        ok: true,
        temp_password: b.new_password ? undefined : pw,
      });
    }
    if (b.action === "disable") {
