...
export default function Docs() {
  return (
    <AppShell title="Project Docs" subtitle="คู่มือระบบ QC Admin ปัจจุบัน — QC Engine v4 (SOP-driven)">
      <article className="card" style={{ maxWidth: 860, lineHeight: 1.8 }}>
        <p style={{ color: "#6b7280", marginTop: 0 }}>อัปเดตล่าสุด มิถุนายน 2026</p>

        {/* 1. QC Engine v4 */}
        <h2>1. QC Engine v4</h2>
        <p>
          ให้คะแนนการตอบของแอดมินอัตโนมัติผ่าน {code("runQc()")}: detectIntent → matchSOP → rubric 8 มิติ → บันทึก{" "}
          {code("qc_scores")} + {code("qc_score_details")} → Telegram alert (fail/fatal)
        </p>
        <ul>
          <li>
            น้ำหนักคงที่: Greeting &amp; Closing <b>15</b> · Problem Solving &amp; Accuracy <b>20</b> · Communication
            &amp; Tone <b>20</b> · Response Time <b>10</b>
          </li>
          <li>intent-specific: Upselling 10 (promotion/bonus) · Credit Deposit/Withdraw 10 (ฝาก/ถอน) · KYC 10 (kyc)</li>
          <li>มิติที่ไม่เกี่ยวกับ intent = N/A (ไม่คิดในตัวหาร) แต่บันทึกเหตุผลไว้</li>
          <li>
            <b>Minor</b> (ไม่มีคำลงท้ายสุภาพ/ตอบสั้น/ส่งซ้ำ) → −5 · <b>Fatal</b>{" "}
            (หยาบ/โทษลูกค้า/รับประกันผลพนัน/ปฏิเสธช่วยเหลือ) → 0
          </li>
          <li>
            <b>SLA exception</b>: มี System Event ครอบเวลานั้น → response time floor 80
          </li>
          <li>แต่ละมิติเก็บ evidence (keyword ที่เจอ/ขาด, similarity, เวลาตอบ) ลง {code("qc_score_details")}</li>
        </ul>
        ...
        <ol>
          ...
          <li>
            แต่ละ SOP: topic, question, answer, intent, keywords, required_keywords, forbidden_keywords, escalation
          </li>
          <li>
            ตรวจคุณภาพข้อมูล: {code("npm run audit:sop")} (duplicate topics / empty answer / missing keyword / category
            mismatch / never-matched)
          </li>
        </ol>
        ...
        <ol>
          ...
          <li>
            ปิดใช้งานชั่วคราว = ปุ่ม ON/off (soft, {code("is_active=false")}) · ลบถาวร = {code("?hard=true")}
          </li>
          <li>
        ...
        <p>
          บันทึกช่วงระบบ/ธนาคารล่มที่ {code("/system-events")} — ติ๊ก {code("affects_sla")} + ระบุช่วงเวลา →
          เคสที่ตอบช้าในช่วงนั้น response time จะไม่ถูกหักเต็ม (floor 80) ตรวจผ่าน {code("isSlaException()")} ใน
          qc-runner
        </p>
