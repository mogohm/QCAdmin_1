...
export default function Disputes() {
  ...
  const load = () => {
    ...
    fetch('/api/qc-disputes' + p).then(r => r.json()).then(d => { setRows(d.disputes || []); setCounts(d.counts || []); if (!d.disputes?.find(x => x.id === pick)) setPick(d.disputes?.[0]?.id || null); });
  };
  ...
  const review = async (id, status) => {
    ...
    setMsg(j.ok ? `✓ ${status}` : '⚠️ ' + (j.error || 'error')); setNote(''); setNewScore(''); load(); setTimeout(() => setMsg(''), 2000);
  };
      ...
      <>
        ...
        <section className="grid" style={{ gridTemplateColumns: '320px 1fr', gap: 16, alignItems: 'start' }}>
          ...
          <div className="card" style={{ padding: 8, maxHeight: '70vh', overflow: 'auto' }}>
            {rows.map(x => (
              <div key={x.id} onClick={() => setPick(x.id)} style={{ padding: 10, borderRadius: 10, cursor: 'pointer', marginBottom: 6, background: pick === x.id ? '#eff6ff' : '#fff', border: '1px solid ' + (pick === x.id ? '#bfdbfe' : '#eef3f8') }}>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}><b style={{ fontSize: 13 }}>{x.admin_name || '—'}</b><span className={`score ${sc(x.old_score)}`}>{x.old_score}</span></div>
                <div className="muted" style={{ fontSize: 11 }}>{x.intent} · {new Date(x.created_at).toLocaleDateString('th-TH')}</div>
                <div style={{ fontSize: 12, color: '#555', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{x.reason}</div>
              </div>
            ))}
            {!rows.length && <div className="muted" style={{ padding: 16, textAlign: 'center' }}>ไม่มี dispute</div>}
          </div>

          {/* detail */}
          <div className="card">
            {!d ? <div className="muted">เลือก dispute จากด้านซ้าย</div> : (
              <>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 8 }}>
                  <div><h3 style={{ margin: 0 }}>{d.admin_name}</h3><span className="muted" style={{ fontSize: 12 }}>{d.intent} · {new Date(d.created_at).toLocaleString('th-TH')}</span></div>
                  <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                    <span className={`score ${sc(d.old_score)}`} style={{ fontSize: 22 }}>{d.old_score}</span>
                    {d.new_score != null && <><span className="muted">→</span><span className={`score ${sc(d.new_score)}`} style={{ fontSize: 22 }}>{d.new_score}</span></>}
                ...
                {d.status === 'pending' && (
                  <div style={{ marginTop: 12, borderTop: '1px solid #eef3f8', paddingTop: 12 }}>
                    <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
                      <input placeholder="คะแนนใหม่ (0-100)" type="number" min="0" max="100" value={newScore} onChange={e => setNewScore(e.target.value)} style={{ width: 160, margin: 0 }} />
                      <input placeholder="หมายเหตุ Manager" value={note} onChange={e => setNote(e.target.value)} style={{ flex: 1, margin: 0, minWidth: 180 }} />
                    </div>
