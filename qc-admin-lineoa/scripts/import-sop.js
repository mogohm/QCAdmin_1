...
const CATEGORY_NAMES = {
  register: 'สมัครสมาชิก', deposit: 'ฝากเงิน', withdraw: 'ถอนเงิน', kyc: 'ยืนยันตัวตน/บัญชี',
  promotion: 'โปรโมชั่น', bonus: 'โบนัส', jackpot: 'แจ็คพอต/Leaderboard', poker: 'โป๊กเกอร์/วิธีเล่น',
  tournament: 'ทัวร์นาเมนต์', technical_issue: 'ปัญหาทางเทคนิค/ระบบ', escalation: 'ส่งต่อทีมงาน',
  closing: 'ปิดเคส/แจ้งรอ', general: 'ทั่วไป',
};
...
const TH_STOP = new Set(['และ','หรือ','ที่','ให้','ได้','เป็น','ของ','กับ','จะ','การ','ใน','ค่ะ','ครับ','นะคะ','นะครับ','คุณ','ลูกค้า','แอดมิน','รบกวน','สำหรับ','กรณี','โดย','แล้ว','คือ','มี','จาก','ไป','มา','ขอ','ตาม']);
...
function genRequired(topic, answer) {
  ...
  for (const w of en) if (!['the','and','you','for','your','please','wait','com','www','https','http'].includes(w)) req.add(w);
...
function readExcel() {
  ...
  let kbSheet = null, qCol = null, aCol = null;
  for (const name of wb.SheetNames) {
    ...
    if (qi >= 0 && ai >= 0) { kbSheet = name; qCol = header[qi]; aCol = header[ai]; break; }
  }
...
function build() {
  ...
  const categories = [...catSet].map(code => ({
    code, name: CATEGORY_NAMES[code] || code,
  ...
  const fatal_rules = [
    { code: 'FATAL-RUDE', name: 'ใช้คำหยาบ/ดูถูกลูกค้า', description: 'แอดมินใช้คำไม่สุภาพหรือดูถูกลูกค้า',
      patterns: ['โง่','บ้า','เรื่องมาก','รำคาญ','หัดอ่าน','ทำไมไม่อ่าน','ปัญญาอ่อน'], applies_to: null },
    { code: 'FATAL-BLAME', name: 'โทษ/ตำหนิลูกค้า', description: 'โยนความผิดให้ลูกค้า',
      patterns: ['เป็นความผิดของลูกค้า','ลูกค้าผิดเอง','ก็บอกแล้ว','ไม่ใช่ความผิดเรา'], applies_to: null },
    { code: 'FATAL-GUARANTEE', name: 'รับประกันผลการพนัน/เกินจริง', description: 'การันตีว่าจะได้เงิน/ชนะแน่นอน (ผิดนโยบาย)',
      patterns: ['การันตีได้เงิน','รับประกันได้เงิน','ชนะแน่นอน','ได้เงินแน่นอน100','การันตีกำไร'], applies_to: null },
    { code: 'FATAL-DISMISS', name: 'ปฏิเสธ/ไม่ช่วยเหลือ', description: 'บอกปัดว่าไม่รู้/ไม่ช่วย โดยไม่ส่งต่อ',
      patterns: ['ไม่รู้','ไม่ทราบเหมือนกัน','ช่วยไม่ได้','ไปถามที่อื่น','ไม่ใช่หน้าที่'], applies_to: null },
  ];
...
async function insertToDB(data) {
  ...
  for (const part of mig.split(/;\s*\n/).map(x => x.trim()).filter(Boolean)) await db(part + ';').catch(e => { if (!/already exists/i.test(e.message)) throw e; });
  ...
  console.log(`💾 insert: ${data.categories.length} categories, ${n} scripts, ${data.intent_patterns.length} patterns, ${data.fatal_rules.length} fatal rules`);
}
...
(async () => {
  ...
  console.log(`หมวด (${data.categories.length}):`, data.categories.map(c => `${c.code}=${data.stats.intents[c.code]}`).join(', '));
