...
console.log("\n== 6) worker ตายกลางคัน → job อยู่ในสถานะกู้คืนได้ ==");
{
  ...
  // orphan auto-recovery: running + worker ตาย + updated_at ค้าง → ห้ามค้าง running ถาวร
  const old = new Date(Date.now() - 10 * 60000).toISOString();
  const fresh = new Date(Date.now() - 30 * 1000).toISOString();
  t("orphan (auth ล่าสุด) → blocked_auth", orphanJobRecovery({ status: "running", updated_at: old, error_code: "LINE_SESSION_EXPIRED" }, { workerOnline: false }) === "blocked_auth");
  t("orphan (ไม่ใช่ auth) → pending (job เดิม)", orphanJobRecovery({ status: "running", updated_at: old }, { workerOnline: false }) === "pending");
  t("running แต่ updated_at ยังสด → ไม่แตะ", orphanJobRecovery({ status: "running", updated_at: fresh }, { workerOnline: false }) === null);
  t("running + worker online → ไม่แตะ", orphanJobRecovery({ status: "running", updated_at: old }, { workerOnline: true }) === null);
  t("done/pending ไม่ใช่ orphan", orphanJobRecovery({ status: "done", updated_at: old }, {}) === null && orphanJobRecovery({ status: "pending", updated_at: old }, {}) === null);
  t("worker startup เรียก recoverOrphanRunningJobs", /await recoverOrphanRunningJobs\(\)/.test(src));
}

console.log("\n== 7) ป้าย VALID ค้าง: ผลตรวจเก่าเกิน 5 นาที ห้ามเชื่อ ==");
{
  const now = Date.now();
  const at = (min) => new Date(now - min * 60000).toISOString();
  t("valid ตรวจเมื่อ 1 นาที → valid", effectiveSessionStatus("valid", at(1), now) === "valid");
  t("valid ตรวจเมื่อ 6 นาที → unknown (ห้ามโชว์ ✅)", effectiveSessionStatus("valid", at(6), now) === "unknown");
  t("valid ไม่มี checked_at → unknown", effectiveSessionStatus("valid", null, now) === "unknown");
  t("expired ไม่หมดอายุเอง (ยัง expired)", effectiveSessionStatus("expired", at(60), now) === "expired");
  const ui = fs.readFileSync(path.join(__dirname, "..", "app", "scraper", "page.js"), "utf8");
  t("UI ใช้ effectiveSessionStatus (กันป้ายเขียวค้าง)", /effectiveSessionStatus\(/.test(ui));
}
...
console.log("\n== 9) ghost blocked_auth: create/cancel job ใหม่ ต้องเคลียร์ blocked_auth เก่า ==");
{
  ...
  // end-to-end: user สร้าง job ใหม่ทั้งที่มี blocked_auth เก่า → เก่าถูก cancel → login แล้วต้องไม่ปลุกเก่า
  {
    // 1) มี blocked_auth เก่า · 2) POST สร้างใหม่ = SQL cancel (สถานะเก่า → cancelled)
    const oldAfterCreate = "cancelled"; // ผลของ UPDATE ... WHERE status IN (...,'blocked_auth')
    // 3) requeue ตอน login มองเฉพาะ status==='blocked_auth' เท่านั้น → cancelled ไม่เข้าเงื่อนไข
    const wouldResume = authRestoredTransition(oldAfterCreate) === "pending";
    t("blocked_auth เก่าไม่ resume หลัง user สร้าง job ใหม่ (โดนตั้งใจ)", wouldResume === false);
    // requeueBlockedAuthJobs กรอง status==='blocked_auth' เท่านั้น (ไม่กวาด cancelled)
    const src = fs.readFileSync(path.join(__dirname, "..", "scraper.js"), "utf8");
    const rq = src.slice(src.indexOf("async function requeueBlockedAuthJobs"), src.indexOf("async function recoverOrphanRunningJobs"));
    t("requeue กรองเฉพาะ status==='blocked_auth' (cancelled ไม่ถูกปลุก)", /j\.status === "blocked_auth"/.test(rq));
  }
}
