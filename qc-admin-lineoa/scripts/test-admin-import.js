...
const keep = [
  'PK', 'pk', 'PK-', 'PK_', 'PK-PANG', 'PK Fern', 'PK - บีท', 'PK - ชื่อไทย',
  'ᴘᴋ - ᴋᴏɴɢ',        // small caps
...
console.log('\n===== ไม่ใช่ PK (ต้อง SKIP) =====');
const skip = ['Download', 'Auto-response', 'Read', 'logo', '(hourglass not done)', 'ANne', 'HARP', 'TEEW',
  'QC Test Bot', 'ยอดเงินกำลังอัปเดต', 'The photo in the message', 'N-Nick', 'FOMYFOAM895'];
...
const seen = new Set(); let imported = 0, skipped = 0, duplicated = 0;
for (const name of lines) {
  if (!isPkName(name)) { skipped++; continue; }
  const norm = normalizeAdminName(name).toLowerCase();
  if (seen.has(norm)) { duplicated++; continue; }
  seen.add(norm); imported++;
}
