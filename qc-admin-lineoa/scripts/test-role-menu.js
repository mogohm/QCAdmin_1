...
const hrefs = (role) =>
  filterMenuByPermissions(user(role), MENU).map((m) => m.href);
const sees = (role, href) => hrefs(role).includes(href);

console.log(
  "== role menu (unit: filterMenuByPermissions + canViewRoute ของจริง) ==",
);

// ---- filterMenuByPermissions ----
ok(
  "system_admin เห็นทุกเมนู",
  filterMenuByPermissions(user("system_admin"), MENU).length === MENU.length,
);
...
ok(
  "marketing เห็น /marketing-dashboard",
  sees("marketing", "/marketing-dashboard"),
);
...
// ---- canViewRoute ----
ok(
  "canViewRoute(sysadmin, /system/roles) = true",
  canViewRoute(user("system_admin"), "/system/roles"),
);
ok("canViewRoute(null, /) = false", !canViewRoute(null, "/"));
ok(
  "canViewRoute(admin, /system/users) = false",
  !canViewRoute(user("admin"), "/system/users"),
);
ok(
  "canViewRoute(admin, /admin-dashboard) = true",
  canViewRoute(user("admin"), "/admin-dashboard"),
);
ok(
  "canViewRoute(marketing, /chat/Uxxx) = false",
  !canViewRoute(user("marketing"), "/chat/Uxxxx"),
);
ok(
  "canViewRoute(manager, /chat/Uxxx) = true",
  canViewRoute(user("manager"), "/chat/Uxxxx"),
);
ok(
  "canViewRoute(marketing, /sop) = false",
  !canViewRoute(user("marketing"), "/sop"),
);
...
if (BASE) {
  ...
  for (const [u, p, role] of [
    ...
    if (m?.permissions) {
      const sameSet =
        m.permissions.slice().sort().join(",") ===
        ROLE_PERMS[role].slice().sort().join(",");
