...
console.log("\n===== 2) scraper-live.bat ตั้งค่า UTF-8 ครบ =====");
{
  const bat = fs.readFileSync(path.join(__dirname, "../scraper-live.bat"), "utf8");
  ok("chcp 65001 ก่อน node", bat.includes("chcp 65001"));
  ok("ตั้ง [Console]::OutputEncoding UTF8", bat.includes("[Console]::OutputEncoding"));
  ok("ตั้ง $OutputEncoding UTF8", bat.includes("$OutputEncoding"));
  ok("เลือก pwsh (PowerShell 7) ก่อน", bat.includes("where pwsh"));
  ok("พิมพ์ Console engine", bat.includes("Console engine:"));
  ok("WP5.1 เขียน log ด้วย Add-Content -Encoding utf8 (Tee เขียน UTF-16)", bat.includes("Add-Content") && bat.includes("-Encoding utf8"));
