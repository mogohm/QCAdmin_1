...
for (const [msg, exp] of intentCases) {
  ...
  ok(`"${msg.slice(0, 28)}" → ${d.intent} (conf ${d.confidence})`, d.intent === exp, exp !== d.intent ? `คาดว่า ${exp}` : '');
}
...
for (const msg of matchCases) {
  ...
  ok(`match "${msg}" → "${m.sop ? m.sop.topic.slice(0, 30) : 'none'}" (${m.method}, conf ${m.confidence})`, m.sop && m.confidence > 20);
}

console.log('\n===== 4) QC SCORING (8 มิติ) =====');
const good = scoreReply({
  customerText: 'ขอลิงก์ฝากเงินหน่อยค่ะ', adminText: 'วิธีเติมเงิน Auto เช็คที่เมนูฝากทุกครั้ง ลิงก์ https://bit.ly/xxx ค่ะ ยินดีให้บริการนะคะ',
  responseSeconds: 60, sops, fatalRules,
});
...
const slow = scoreReply({ customerText: 'ถอนเงินยังไง', adminText: 'ถอนผ่านเมนูถอนค่ะ', responseSeconds: 1800, sops, fatalRules });
ok('ตอบช้า → responseTime ต่ำ', slow.dimensions.responseTime < 80, `rt ${slow.dimensions.responseTime}`);

console.log('\n===== 5) FATAL RULES =====');
const rude = scoreReply({ customerText: 'ถอนเงินยังไง', adminText: 'โง่จริงๆ หัดอ่านเองบ้าง', responseSeconds: 30, sops, fatalRules });
ok('คำหยาบ → fatal & score 0', rude.isFatal && rude.finalScore === 0, `score ${rude.finalScore}`);
const dismiss = scoreReply({ customerText: 'ฝากเงินไม่เข้า', adminText: 'ไม่รู้ ไปถามที่อื่น', responseSeconds: 30, sops, fatalRules });
ok('ปฏิเสธช่วยเหลือ → fatal', dismiss.isFatal, `score ${dismiss.finalScore}`);

console.log('\n===== 6) COACHING =====');
const coach = generateCoaching({ customerText: 'ถอนเงินยังไง', adminText: 'ถอนเองสิ', scoreResult: scoreReply({ customerText: 'ถอนเงินยังไง', adminText: 'ถอนเองสิ', responseSeconds: 600, sops, fatalRules }), sop: matchSOP('ถอนเงินยังไง', sops).sop });
ok('สร้าง coaching เมื่อคะแนนต่ำ', coach && coach.reasons.length > 0, coach ? `${coach.reasons.length} เหตุผล` : 'null');
