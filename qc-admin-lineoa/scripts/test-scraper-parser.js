...
ok(
  "ไม่มี system message หลุดเข้ามา",
  !msgs.some((m) => /เข้าร่วมการสนทนา/.test(m.message_text)),
);
ok(
...
ok(
  "ตรวจ sticker จาก data-mtype",
  !!sticker,
  sticker ? sticker.message_text : "ไม่เจอ",
);
ok(
  "ข้อความ text ปกติ = type text",
  adm.filter((m) => m.message_type === "text").length === 2,
);
...
ok(
  "date separator ไม่ถูกนับเป็นข้อความ + system ถูกข้าม → 4 ข้อความ",
  tmsgs.length === 4,
  `got ${tmsgs.length}`,
);
ok(
...
const p1 = pairs[0];
ok(
  "คู่ 1: รวมคำถามลูกค้า 2 ข้อความ",
  /ขอลิงก์ฝากเงิน/.test(p1.customer_text) && /วิธีไหน/.test(p1.customer_text),
);
ok(
  "คู่ 1: รวม bubble admin 2 อันเป็นคำตอบเดียว",
  /เมนูฝาก/.test(p1.admin_text) && /ตรวจสอบยอด/.test(p1.admin_text),
);
ok("คู่ 1: admin_name = PK - Mei", p1.admin_name === "PK - Mei");
ok(
  "คู่ 1: response_seconds = 60 (10:31→10:32)",
  p1.response_seconds === 60,
  String(p1.response_seconds),
);
ok("คู่ 1: reply_group_id มีค่า", !!p1.reply_group_id);
const p2 = pairs[1];
ok(
  "คู่ 2: customer = ขอบคุณ, admin = sticker",
  /ขอบคุณ/.test(p2.customer_text) && p2.message_type === "sticker",
);
