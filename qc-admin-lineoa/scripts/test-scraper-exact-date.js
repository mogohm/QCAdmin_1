...
console.log("\n===== 9) MEDIUM 2: direct/history/too_old classification =====");
{
  const from = "2026-07-04";
  const to = "2026-07-05";
  const NOW = new Date(2026, 6, 6, 12, 0, 0); // today 07-06
  ok("07-05 ในช่วง → direct", core.classifyCandidate("2026-07-05", from, to, NOW) === "direct");
  ok("07-04 ขอบล่างช่วง → direct", core.classifyCandidate("2026-07-04", from, to, NOW) === "direct");
  ok("Today (07-06 > to) → history", core.classifyCandidate("Today", from, to, NOW) === "history");
  ok("07-03 (< from) → too_old", core.classifyCandidate("2026-07-03", from, to, NOW) === "too_old");
  ok("label มั่ว → unknown (เก็บไว้ก่อน)", core.classifyCandidate("???", from, to, NOW) === "unknown");
}
