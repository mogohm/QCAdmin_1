...
for (const k of required)
  ok(`payload มี field: ${k}`, payload[k] !== undefined);
...
ok(
  "คู่ซ้ำถูกตรวจจับ (2 unique, 2 ซ้ำ)",
  seen.size === 2 && dup === 2,
  `unique=${seen.size} dup=${dup}`,
);
...
(async () => {
  if (API && KEY) {
    try {
      ...
      ok(
        "POST /api/admin/log-reply ok",
        j.ok === true,
        j.error || JSON.stringify(j).slice(0, 80),
      );
      ...
      ok(
        "create qc_scores (มี qc_score_id)",
        !!j.qc_score_id,
        JSON.stringify(j).slice(0, 90),
      );
      ok(
        "runQc ทำงาน (final_score เป็นตัวเลข)",
        typeof j.final_score === "number",
      );
    ...
    console.log(
      "⏭️  ข้าม live test — ไม่มี QC_API_URL + QC_API_KEY (ทดสอบ payload/dedup แบบ offline แล้ว)",
    );
  }
